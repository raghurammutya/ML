#!/usr/bin/env python3
"""
Enhanced Tick Monitor - StocksBlitz SDK Complete Testing (FIXED VERSION)

This version dynamically discovers available instruments from the backend
instead of using hardcoded symbols. Works with both live and mock data.

Features:
- Authentication (JWT)
- Dynamic instrument discovery from backend
...
class BackendDataFetcher:
    ...
    def __init__(self, api_url: str):
        ...
        self.snapshot_url = f"{api_url}/monitor/snapshot"

    def get_snapshot(self) -> Dict:
        """Get monitor snapshot with all available instruments"""
        try:
            response = requests.get(self.snapshot_url, timeout=10)
            response.raise_for_status()
            return response.json()
        except Exception as e:
            print_error(f"Failed to fetch backend snapshot: {e}")
            return {}

    def get_underlying(self) -> Optional[Dict]:
        """Get underlying index data"""
        snapshot = self.get_snapshot()
        return snapshot.get('underlying')

    def get_options(self, min_volume: int = 0, expiry_filter: Optional[str] = None) -> List[Dict]:
        """Get available options, optionally filtered"""
        snapshot = self.get_snapshot()
        options = snapshot.get('options', {})

        result = []
        for token, opt_data in options.items():
            # Apply filters
            if min_volume > 0 and opt_data.get('volume', 0) < min_volume:
                continue
            if expiry_filter and expiry_filter not in opt_data.get('tradingsymbol', ''):
                continue
    ...
    def get_atm_options(self, spot_price: float, expiry_code: str = "25N04",
                       strikes_below: int = 3, strikes_above: int = 3) -> Dict[str, List[str]]:
        ...
        for i in range(-strikes_below, strikes_above + 1):
            ...
            call_symbol = f"NIFTY{expiry_code}{int(strike)}CE"
            put_symbol = f"NIFTY{expiry_code}{int(strike)}PE"

            # Verify they exist in backend
            options = self.get_options(expiry_filter=expiry_code)
            available_symbols = {opt['tradingsymbol'] for opt in options}

            if call_symbol in available_symbols:
                calls.append(call_symbol)
            if put_symbol in available_symbols:
                puts.append(put_symbol)
...
def display_underlying_data(client: TradingClient, symbol: str, exporter: DataExporter, backend_data: Optional[Dict] = None):
    """Display underlying (NIFTY spot) data"""
    print_section(f"📊 Underlying: {symbol}")

    try:
        # If we have backend data, show it directly
        if backend_data:
            price = backend_data.get('price', 0)
            volume = backend_data.get('volume', 0)
            print(f"{Colors.BOLD}Price:{Colors.ENDC} {Colors.OKGREEN}{price:,.2f}{Colors.ENDC}")
...
def display_option_chain_heatmap(client: TradingClient, spot_price: float,
                                strike_range: List[float], expiry_code: str = "25N04"):
...
def run_tick_monitor(client: TradingClient, backend_fetcher: BackendDataFetcher,
    ...
    # Get underlying data from backend
    print_info("Fetching available instruments from backend...")
    underlying_data = backend_fetcher.get_underlying()

    if not underlying_data:
        print_warning("No underlying data found, using fallback")
        spot_price = 25700
        underlying_symbol = "NIFTY 50"
    else:
        underlying_symbol = underlying_data.get('symbol', 'NIFTY 50')
        spot_price = underlying_data.get('price', 25700)
        print_success(f"Found {underlying_symbol} at {spot_price:.2f}")

    # Get available options from backend (Nov 4 expiry)
    atm_options = backend_fetcher.get_atm_options(spot_price, expiry_code="25N04",
                                                  strikes_below=3, strikes_above=3)

    monitored_options = atm_options['calls'] + atm_options['puts']
    print_success(f"Found {len(monitored_options)} available options around ATM")
    ...
    try:
        while time.time() - start_time < duration_seconds:
            ...
            underlying_data = backend_fetcher.get_underlying()
            if underlying_data:
                spot_price = underlying_data.get('price', spot_price)

            # Display underlying
            display_underlying_data(client, underlying_symbol, exporter, underlying_data)
            ...
            if iteration % 3 == 0:
                ...
                display_option_chain_heatmap(client, spot_price, strike_range, "25N04")
...
def main():
    """Main function"""
    print_header("StocksBlitz SDK - Enhanced Tick Monitor (FIXED)")
    ...
    backend_fetcher = BackendDataFetcher(API_URL)

    # Verify backend connectivity
    print_section("Backend Connectivity")
    snapshot = backend_fetcher.get_snapshot()
    if not snapshot:
        print_error("Failed to connect to backend")
        return 1

    options_count = len(snapshot.get('options', {}))
    print_success(f"Backend connected! Found {options_count} instruments")
    ...
    try:
        run_tick_monitor(client, backend_fetcher, duration_seconds=300)
    except Exception as e:
