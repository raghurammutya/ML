#!/usr/bin/env python3
"""
Focused options backfill for ATM ±10 strikes using CORRECT tokens from ticker service subscriptions.
This fixes the 502 error issue by using actual subscribed tokens instead of database tokens.
"""
import asyncio
import sys
import re
...
async def main():
    """Main options backfill execution using ticker service subscriptions."""
    ...
    # Query instrument_registry for Nov 11 options (not subscriptions)
    print(f"📋 Querying instrument_registry for NIFTY Nov 11 options...")
    ...
    print(f"✅ Found {len(strikes_data)} strikes with subscribed instruments\n")
        ...
        for strike_idx, (strike, tokens) in enumerate(sorted(strikes_data.items()), 1):
            ...
            if call_candles or put_candles:
                bars_inserted = await insert_option_strike_bars(
                    pool, "NIFTY", datetime.fromisoformat(TARGET_EXPIRY_STR).date(), strike, call_candles, put_candles
                )
    ...
    async with pool.acquire() as conn:
        result = await conn.fetch("""
            ...
            ORDER BY date
        """, datetime.fromisoformat(TARGET_EXPIRY_STR).date())
